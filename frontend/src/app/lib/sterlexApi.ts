/**
 * Sterlex API client — all requests to the Node.js backend.
...
export class SterlexApiError extends Error {
    status: number;
    code: string | null;

    constructor(args: { message: string; status: number; code?: string | null }) {
        super(args.message);
        this.name = "SterlexApiError";
...
export function isMfaRequiredError(error: unknown) {
    return (
        error instanceof SterlexApiError &&
        error.status === 403 &&
...
async function toApiError(response: Response, path: string) {
    ...
    try {
        ...
        devLog("[sterlex-api] non-ok response", {
            path,
            status: response.status,
            code: parsed.code,
            detail: parsed.detail,
        });
        return new SterlexApiError({
            status: response.status,
        ...
        devLog("[sterlex-api] non-ok non-json response", {
            path,
            status: response.status,
